...
def life_crear(mapa):
    ...
    for celula in mapa:
        filas_tablero=[]
        for caracter in celula:
            if caracter=="#":
                filas_tablero.append(True)
            else:
                filas_tablero.append(False)
        tablero.append(filas_tablero)
    return tablero
    ...
    for fila in life:
        ...
        for elemento in fila:
            if elemento:
                ...
                filas_tablero.append(".")
        tablero.append("".join(filas_tablero))
    return tablero
    ...
    for fila in range(f-1,f+2):
        for columna in range(c-1,c+2):
            if fila==f and columna==c:
                continue
            if life[fila%len(life)][columna%len(life[0])]:
                adyacentes+=1
    return adyacentes
...
def celda_siguiente(life, f, c):
    ...
    """
    esta_viva=life[f][c]
    cantidad_de_adyacentes=cant_adyacentes(life, f, c)
    if esta_viva and cantidad_de_adyacentes==2 or cantidad_de_adyacentes==3:
        return True
    if cantidad_de_adyacentes==3:
        return True
    return False
